...
tests_3hop = generate_metaqa_tests(tests_path="./metaqa_data/hop_reasoning/metaqa-3hop/ntm/qa_test.txt",
                                   entities=[], relations=[], attributes=[], is_classic_metaqa=True)
...
for i in range(len(tests)):
    ...
    # print results
    put_results_on_files(hop_index=2, model_name=model_name, successful_compilations=successful_compilations, correct_responses=correct_responses,
                         unsuccessful_compilations=unsuccessful_compilations, wrong_responses=wrong_responses, number_of_tests=number_of_tests, experiment_total_cost=experiment_total_cost, end_time=end_time)
